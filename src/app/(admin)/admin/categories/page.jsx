...
import { FiFolder, FiEdit2, FiTrash2, FiPlus, FiSave, FiX, FiLoader, FiAlertCircle, FiInfo, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                >
                    {showForm ? (
                        <>
                            <FiX className="mr-1" /> Ẩn form
                        </>
                    ) : (
                        <>
                            <FiPlus className="mr-1" /> Thêm danh mục mới
                        </>
                    )}
                </button>
            </div>
                        ...
                        {categories.length > itemsPerPage && (
                            <nav aria-label="Phân trang" className="flex items-center justify-center mt-6">
                                <div className="flex items-center space-x-1 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-1">
                                    ...
                                    {/* Nút Tiếp */}
                                    <button
                                        onClick={() => setCurrentPage(Math.min(Math.ceil(categories.length / itemsPerPage) - 1, currentPage + 1))}
                                ...
                                <div className="ml-4 text-sm text-gray-600 dark:text-gray-400">
                                    Hiển thị <span className="font-medium text-orange-600 dark:text-orange-400">{(currentPage * itemsPerPage) + 1}-{Math.min((currentPage + 1) * itemsPerPage, categories.length)}</span> / <span className="font-medium">{categories.length}</span> danh mục
                                </div>
                            </nav>
                        )}
                    </>
