...
import { FiShoppingCart, FiAward, FiHeart, FiClock, FiStar } from 'react-icons/fi';
...
const CuratedCollections = ({ products = [] }) => {
    ...
    // Chỉ hiển thị sách có đánh giá
    const bestsellers = products.filter(product => product.averageRating > 0);

    if (bestsellers.length === 0) {
        return (
            <div className="text-center py-10 bg-white dark:bg-gray-800 rounded-lg shadow text-gray-500 dark:text-gray-400">
                <p>Hiện không có sách bán chạy. Vui lòng quay lại sau!</p>
            </div>
        );
    }
